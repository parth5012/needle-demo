...
export function App() {
  ...
  const handleInputChange = (field: keyof ArtisanOnboardingForm, value: any) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSubmitSuccess(`Profile registered successfully! ID: ${data.artisan_id} (${data.verification_status})`);
    } catch (err: any) {
      setError(err.message || 'Error submitting onboarding profile');
    } finally {
  ...
  const renderConfidenceBadge = (fieldKey: string) => {
    ...
    return (
      <span
        ...
        className={`inline-flex items-center gap-1 px-1.5 py-0.5 text-xs font-semibold rounded border ${color}`}
      >
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 flex flex-col font-sans">
      ...
      <header className="border-b border-slate-800 bg-slate-950/80 backdrop-blur sticky top-0 z-50 px-6 py-3.5 flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          <div className="flex items-center gap-2 px-3 py-1 rounded-lg bg-slate-800/80 border border-slate-700 text-xs text-slate-300">
            <div className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
            <span>Needle 3 Local Inference Engine</span>
          </div>
          <a
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-6 grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        {/* Right Column: Auto-Filled Structured Form */}
            ...
            <div className="flex items-center justify-between pb-4 border-b border-slate-800 mb-5">
              <div>
                <h2 className="font-bold text-base text-white flex items-center gap-2">
                  ...
                  Artify Bharat: Artisan Onboarding Record
                </h2>
                <p className="text-xs text-slate-400 mt-0.5">
                  Verify & adjust auto-extracted fields before publishing to National Crafts Registry
                </p>
              </div>
              ...
              <div className="flex bg-slate-900 border border-slate-800 rounded-lg p-0.5 text-xs">
                <button
                  ...
                  className={`px-3 py-1 rounded-md transition ${activeTab === 'form' ? 'bg-slate-800 text-white font-medium shadow-sm' : 'text-slate-400 hover:text-slate-200'}`}
                >
                  Form UI
                </button>
                <button
                  onClick={() => setActiveTab('json')}
                  className={`px-3 py-1 rounded-md transition flex items-center gap-1 ${activeTab === 'json' ? 'bg-slate-800 text-white font-medium shadow-sm' : 'text-slate-400 hover:text-slate-200'}`}
                >
                ...
                <button
                  ...
                  className={`px-3 py-1 rounded-md transition ${activeTab === 'entities' ? 'bg-slate-800 text-white font-medium shadow-sm' : 'text-slate-400 hover:text-slate-200'}`}
                >
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-1">
                        <label className="text-xs text-slate-300 font-medium flex items-center gap-1">
                          <Hash className="w-3 h-3 text-slate-400" /> 4-Digit Security PIN
                        </label>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => handleInputChange('pehchan_id', e.target.value)}
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-1">
                        <label className="text-xs text-slate-300 font-medium">TRIFED Registration ID</label>
                        {renderConfidenceBadge('trifed_id')}
                      </div>
                      <input
                        type="text"
                        value={formData.trifed_id || ''}
                        onChange={(e) => handleInputChange('trifed_id', e.target.value)}
                    ...
                    <div>
                      ...
                      <select
                        ...
                        onChange={(e) => handleInputChange('primary_dialect', e.target.value)}
                        className="w-full px-3 py-2 bg-slate-900 border border-slate-800 rounded-lg text-xs text-slate-100 focus:border-amber-500 focus:outline-none"
                      >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => handleInputChange('craft_category', e.target.value)}
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => handleInputChange('gi_tag_name', e.target.value)}
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => handleInputChange('years_experience', e.target.value ? parseInt(e.target.value) : null)}
                        placeholder="e.g. 18"
                        className="w-full px-3 py-2 bg-slate-900 border border-slate-800 rounded-lg text-xs text-slate-100 focus:border-amber-500 focus:outline-none"
                      />
                    </div>
